...
class EstimateService:
    ...
    def create_estimate(self, estimate_data: dict) -> Estimate:
    ...
    def get_estimate(self, estimate_number: int) -> Optional[Estimate]:
        return self.repository.get_by_id(estimate_number)

    def list_estimates(self, skip: int = 0, limit: int = 100) -> List[Estimate]:
        return self.repository.list(skip, limit)

    def update_estimate(self, estimate_number: int, estimate_data: dict) -> Optional[Estimate]:
    ...
    def delete_estimate(self, estimate_number: int) -> bool:
        return self.repository.delete(estimate_number)

    def get_customer_estimates(self, customer_code: str, skip: int = 0, limit: int = 100) -> List[Estimate]:
        return self.repository.get_by_customer_code(customer_code, skip, limit)

    def get_estimates_by_date_range(self, start_date: datetime, end_date: datetime, skip: int = 0, limit: int = 100) -> List[Estimate]:
        return self.repository.get_by_date_range(start_date, end_date, skip, limit)

    def calculate_total_amount_for_customer(self, customer_code: str) -> float:
        estimates = self.repository.get_by_customer_code(customer_code)
        return sum(estimate.total_amount for estimate in estimates)

    def mark_estimate_as_ordered(self, estimate_number: int) -> Optional[Estimate]:
    ...
    def validate_estimate(self, estimate: Estimate) -> bool:
